...
for row in data[:2]:
    ...
    discarded_digits = []
    max_iterations = 0
    row_mask = np.ones(len(row), dtype=int)
    ...
    while len(battery_joltages_row) < REQUIRED_BATTERY_SIZE and max_iterations < 50:
        ...
        if len(remaining_row) < REQUIRED_BATTERY_SIZE - len(battery_joltages_row):
            ...
            row_mask[maximum_index] = 0
            print(f"Row mask after discarding: {row_mask}")
            print(f"Row after discarding: {row[row_mask == 1]}")
            ...
            row = remaining_row
            remaining_length -= 1
            # row_mask = row_mask[-len(remaining_row):]
